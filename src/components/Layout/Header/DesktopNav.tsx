...
import CountrySelectionModal from './ContriesModal.tsx';
...
const DesktopNav: FC = () => {
  ...
  const handleModalOpen = (_event: React.MouseEvent<HTMLElement>, subItems: MenuType[]) => {
        ...
        {menu.map((item) => (
          <Grid item key={item.text}>
            <Box sx={{ cursor: 'pointer', position: 'relative' }}>
              <Link
                onMouseEnter={(event) =>
                  !item.triggerModal && handlePopoverOpen(event, item.subItems)
                }
                onClick={(event) => item.triggerModal && handleModalOpen(event, item.subItems)}
              >
          ...
          <Menu anchorEl={anchorElSub} open={Boolean(anchorElSub)} onClose={handlePopoverSubClose}>
            {innerSubItems?.map((innerSubItem) => (
              <MenuItem key={innerSubItem.text}>
                <Link underline='hover' sx={{ color: 'inherit' }} href={innerSubItem.link}>
                  <Typography variant='body1'>
                    {t(`menu.${innerSubItem.text}`, language)}
                  </Typography>
                </Link>
              </MenuItem>
            ))}
